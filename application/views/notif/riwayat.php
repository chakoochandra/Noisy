    ...
    <tbody>
        <?php if ($data) : ?>
            <?php foreach ($data as $i => $row) : ?>
                <tr <?php echo $row->reference == $selectedId ? 'class="bg-success"' : ($row->success == 0 ? 'class="bg-danger"' : ($row->success == 2 ? 'class="bg-warning"' : 'class="bg-default"')) ?>>
                    <td><?php echo $i + $offset ?></td>
                    <td align="left"><?php echo formatDate($row->sent_time, "%d %b %Y %H:%M:%S") ?></td>
        ...
        <?php else : ?>
            <?php echo "<tr>
                <td colspan='7'>Tidak ada Notifikasi</td>
            </tr>"; ?>
        <?php endif ?>
    </tbody>
</table>
