...
if (!function_exists('my_form')) {
    function my_form($data, $options = '', $attributes = [])
    {
        ...
        foreach ($data as $field) {
            ...
            $html .= isset($field['type']) && (in_array($field['type'], ['form_hidden', 'form_checkbox'])) ? '' : '<div class="input-group mb-3 ' . $divClass . ' ' . $collapse . '">';
            $html .= isset($field['type']) ? switch_input($field['type'], $field) : form_input($field);
...
if (!function_exists('form_toggler')) {
    ...
    function form_toggler($data)
    {
        ...
        return '<div class="btn-group btn-group-toggle" data-toggle="buttons">
                    <label class="btn btn-outline-success ' . ($data['value'] ? 'active' : '') . '" style="min-width: 68px;">
                        <input type="radio" name="' . $data['name'] . '" id="' . $data['name'] . '_option0" ' . $onclick0 . ' value="1" autocomplete="off" ' . ($data['value'] ? 'checked=""' : '') . '> ' . $data['options'][0] . '
                    </label>
                    <label class="btn btn-outline-success ' . (!$data['value'] ? 'active' : '') . '" style="min-width: 68px;">
                        <input type="radio" name="' . $data['name'] . '" id="' . $data['name'] . '_option1" ' . $onclick1 . ' value="0" autocomplete="off" ' . (!$data['value'] ? 'checked=""' : '') . '> ' . $data['options'][1] . '
                    </label>
                </div>';
    }
}
