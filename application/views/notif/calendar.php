<div class="callout callout-primary">
	<h5>Notifikasi Court Calendar</h5>

	<?php $range = get_notif_range($selectedType); ?>
	<p>
		Agenda sampai dengan tanggal <?php echo formatDate(add_days_to_date(date('Y-m-d'), $range)) ?>
	</p>
</div>
	...
	<tbody>
		...
		if ($list) {
			...
			foreach ($list as $j => $row) {
				...
				$tanggal = formatDate($row->rencana_tanggal, "%A, %d %b %Y");
				$jam = date('H:i', strtotime($row->rencana_jam));
				echo "	<td>{$tanggal}<br/>{$jam}</td>";
				echo "  <td>" . "{$row->nomor_perkara}<br/>{$row->rencana_agenda}</td>";
